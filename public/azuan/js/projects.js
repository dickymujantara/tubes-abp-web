...
function loadData() {
    ...
    table = $('#table-project').DataTable({
        ...
        "ajax":{ url:urlProjectList, type:"POST", data:{_token:token}, error: function(err) { alert('failed load data'); loadData(); } },
        "columns": [
            ...
            { name: 'project_number', data: 'project_number', render: function (val, type, data, meta)
                {
                    return val ? "<h6>" + val  + "</h6>": "-";
                }
            },
            ...
            { name: 'id', data: 'id', render: function (val, type, data, meta)
                {
                    return '<div class="btn-group btn-group-sm"><a class="btn btn-circle btn-success btn-xs" data-toggle="modal" href="#modal-project" title="Edit" href="javascript:void(0)" onclick=\"editData(\''+ val +'\')"><i class="fa fa-pencil" aria-hidden="true"></i></a></div>' 
                }
            },
        ], 
        columnDefs: [            
