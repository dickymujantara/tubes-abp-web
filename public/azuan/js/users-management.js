...
function loadData() {
    ...
    table = $('#table-users').DataTable({
        ...
        "ajax":{ url:urlUsersList, type:"POST", data:{_token:token}, error: function(err) { alert('failed load data'); loadData(); } },
        "columns": [
            ...
            { name: 'id', data: 'id', render: function (val, type, data, meta)
                {
                    return '<div class="btn-group btn-group-sm"><a class="btn btn-circle btn-success btn-xs" data-toggle="modal" href="#modal-user" title="Edit" href="javascript:void(0)" onclick=\"editData(\''+ val +'\')"><i class="fa fa-pencil" aria-hidden="true"></i></a>' + 
                                    '<a class="btn btn-circle btn-danger btn-xs" data-toggle="tooltip" title="Detail" href="javascript:void(0)" onclick=\"modalDeleteGroup(\''+ val +'\')\"><i class="fa fa-ban" aria-hidden="true"></i></a></div>' 
                }
            },
        ], 
        columnDefs: [            
